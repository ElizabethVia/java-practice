...
public class ArrayWithOutElement {
	...
	public static void main(String[] args) {
		...
		/*
		 * // find out how many targets are in nums int counter = 0; // loop and using
		 * if condition, increase counter for each match for (int n : nums) { if (n ==
		 * target) { counter++; } }
		 * 
		 * System.out.println(counter); //declare new empty array with size that //does
		 * not include target count int[] newArray = new int[nums.length-counter];
		 * //loop nums one more time and add all values that are not target int idx = 0;
		 * for(int n : nums) { if(n != target) { //(!(n == target)) newArray[idx] = n;
